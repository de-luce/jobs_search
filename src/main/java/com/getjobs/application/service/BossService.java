...
import static com.getjobs.application.entity.table.BossJobDataTableDef.BOSS_JOB_DATA;
import static com.getjobs.application.entity.table.BossOptionTableDef.BOSS_OPTION;
...
public class BossService {
    ...
    public List<BossOptionEntity> getOptionsByType(String type) {
        ...
        BossOptionEntity existing = QueryChain.of(bossOptionMapper)
                .where(BOSS_OPTION.TYPE.eq(type))
                .and(BOSS_OPTION.CODE.eq(com.getjobs.worker.utils.Constant.UNLIMITED_CODE))
                .limit(1)
                .one();
        if (existing == null) {
        ...
        if ("city".equals(type) || "industry".equals(type)) {
            ...
            QueryWrapper wrapper = QueryWrapper.create()
                    .where(BOSS_OPTION.TYPE.eq(type))
                    .orderBy("sort_order IS NULL, sort_order ASC, id ASC");
            return bossOptionMapper.selectListByQuery(wrapper);
        }
        return QueryChain.of(bossOptionMapper)
                .where(BOSS_OPTION.TYPE.eq(type))
                .orderBy(BOSS_OPTION.ID.asc())
                .list();
    }
    ...
    public List<BossOptionEntity> getAllOptions() {
        return bossOptionMapper.selectAll();
    }
    ...
    public String getCodeByTypeAndName(String type, String name) {
        BossOptionEntity entity = QueryChain.of(bossOptionMapper)
                .where(BOSS_OPTION.TYPE.eq(type))
                .and(BOSS_OPTION.NAME.eq(name))
                .one();
    ...
    public String getCityCodeByName(String name) {
        BossOptionEntity entity = QueryChain.of(bossOptionMapper)
                .where(BOSS_OPTION.TYPE.eq("city"))
                .and(BOSS_OPTION.NAME.eq(name))
                .one();
        return entity != null ? entity.getCode() : "0";
    }
    ...
    public List<BossIndustryEntity> getAllIndustries() {
        return bossIndustryMapper.selectAll();
    }
    ...
    public BossIndustryEntity getIndustryByCode(Integer code) {
        return bossIndustryMapper.selectOneById(code);
    }
    ...
    public String getIndustryCodeByName(String name) {
        BossIndustryEntity entity = QueryChain.of(bossIndustryMapper)
                .where(BOSS_INDUSTRY.NAME.eq(name))
                .one();
    ...
    public List<BossConfigEntity> getAllConfigs() {
        return bossConfigMapper.selectAll();
    }
    ...
    public BossConfigEntity getConfigById(Long id) {
        return bossConfigMapper.selectOneById(id);
    }
    ...
    public BossConfigEntity getFirstConfig() {
        return QueryChain.of(bossConfigMapper)
                .limit(1)
                .one();
    }
    ...
    public BossConfigEntity updateConfig(BossConfigEntity config) {
        config.setUpdatedAt(LocalDateTime.now());
        bossConfigMapper.update(config);
        return config;
    }
    ...
    public BossConfigEntity saveOrUpdateFirstSelective(BossConfigEntity partial) {
        ...
        bossConfigMapper.update(existing);
        return existing;
    }
    ...
    public boolean existsBossJob(String encryptId, String encryptUserId) {
        ...
        return QueryChain.of(bossJobDataMapper)
                .where(BOSS_JOB_DATA.ENCRYPT_ID.eq(encryptId))
                .and(BOSS_JOB_DATA.ENCRYPT_USER_ID.eq(encryptUserId))
                .limit(1)
                .one() != null;
    }
    ...
    public void updateDeliveryStatus(String encryptId, String encryptUserId, String status) {
        ...
        UpdateChain<BossJobDataEntity> update = UpdateChain.of(bossJobDataMapper)
                .set(BOSS_JOB_DATA.DELIVERY_STATUS, status)
    ...
    public StatsResponse getBossStats(
        ...
        try {
            // 构造与列表相同的筛选条件
            QueryChain<BossJobDataEntity> wrapper = QueryChain.of(bossJobDataMapper);
            ...
            if (StringUtils.hasText(keyword)) {
                wrapper.and(BOSS_JOB_DATA.COMPANY_NAME.like(keyword)
                        .or(BOSS_JOB_DATA.JOB_NAME.like(keyword))
                        .or(BOSS_JOB_DATA.HR_NAME.like(keyword)));
            }
            if (filterHeadhunter) {
                wrapper.and(BOSS_JOB_DATA.HR_POSITION.isNull()
                        .or(BOSS_JOB_DATA.HR_POSITION.notLike("猎头")));
            }
            wrapper.orderBy(BOSS_JOB_DATA.CREATED_AT.desc());

            List<BossJobDataEntity> all = wrapper.list();
