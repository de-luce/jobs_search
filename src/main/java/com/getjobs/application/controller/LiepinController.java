...
public class LiepinController {
    ...
    public Map<String, Object> getAllLiepinConfig() {
        ...
        options.put("experience", liepinService.getOptionsByType("experience"));
    ...
    public LiepinConfigEntity updateConfig(@RequestBody LiepinConfigEntity config) {
        ...
        config.setPubTime(normalizeOptionValue("pubTime", config.getPubTime()));
        config.setWorkYearCode(normalizeOptionValue("experience", config.getWorkYearCode()));
        config.setEduLevel(normalizeOptionValue("degree", config.getEduLevel()));
        config.setJobKind(normalizeOptionValue("jobType", config.getJobKind()));
        config.setCompScale(normalizeOptionValue("scale", config.getCompScale()));
        config.setCompStage(normalizeOptionValue("stage", config.getCompStage()));
        config.setCompKind(normalizeOptionValue("compKind", config.getCompKind()));
        if (config.getSalaryCode() != null && !config.getSalaryCode().contains("$")) {
            String salaryCode = liepinService.getCodeByTypeAndName("salary", config.getSalaryCode());
            if (salaryCode != null && !salaryCode.isEmpty()) {
                config.setSalaryCode(salaryCode);
            }
        }
    ...
    private String normalizeOptionValue(String type, String raw) {
        if (raw == null || raw.isBlank()) {
            return raw;
        }
        String code = liepinService.getCodeByTypeAndName(type, raw.trim());
        return (code != null && !code.isEmpty()) ? code : raw.trim();
    }
