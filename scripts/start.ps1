# Get Jobs 一键启动（Windows）：建库（如需）→ 装前端依赖 → 启动后端
# 用法：
#   .\scripts\start.ps1
#   .\scripts\start.ps1 --init-db
#   .\scripts\start.ps1 --rebuild-front
...
$DistDir = Join-Path $Root 'src\main\resources\dist'
...
foreach ($arg in $args) {
    switch ($arg) {
        ...
        '--rebuild-front' { $RebuildFront = $true }
        { $_ -in '-h', '--help' } {
            Write-Host @'
Get Jobs 一键启动（Windows）
  .\scripts\start.ps1                 正常启动
  .\scripts\start.ps1 --init-db       仅初始化数据库后退出
  .\scripts\start.ps1 --rebuild-front 先 npm run build:prod 再启动
'@
            exit 0
        }
        default {
            Write-Error "未知参数: $arg（支持 --init-db / --rebuild-front）"
            exit 1
        }
    }
}
...
function Ensure-Front {
    ...
    if ($RebuildFront -or -not (Test-Path $DistDir)) {
        if ($RebuildFront) {
            Write-Log '重新构建前端静态资源...'
            Push-Location $FrontDir
            try {
                npm run build:prod
            } finally {
            ...
            Write-Warn '未找到 src/main/resources/dist，尝试构建前端...'
            Push-Location $FrontDir
            try {
                npm run build:prod
            } catch {
                Write-Warn '前端构建失败，将依赖后端自动拉起 npm run dev'
            } finally {
                Pop-Location
            }
        }
...
function Start-Backend {
    ...
    Write-Log '管理页默认 http://127.0.0.1:6866 （后端会尝试自动打开）'
